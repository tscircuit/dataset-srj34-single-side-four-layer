...
51 versioned SimpleRouteJson benchmark inputs for via-in-pad, four-layer autorouting, curated for boards with components on top. This public repository was provisioned through `tscircuit/create-repo`.
...
| [Pedometer contribution](originals/pedometer.json) | 1 | `b07fa8a96acc6cf097b14de128f76ee5acf866d0` |
...
The first 50 sample names and their order remain unchanged; `pedometer` is appended as case 51. This is an explicit dataset revision; consumers must adopt its new commit and rerun both sides of a benchmark comparison. The browser labels the corrected input and offers its benchmark file and preserved original separately.

## Deterministic selection

Curation retains all 25 unique eligible cases among the 101 pinned dataset01/dataset-srj18 originals. It then selects the first 25 eligible unique genuine wrapped bug reports in numeric fixture order. Report wrappers must contain an actual report ID; placeholder IDs are excluded. The original curation never invokes a router or consults completion/DRC results. Explicitly contributed regression inputs are then appended in `contributions.json` order after the same eligibility and duplicate checks. The pedometer was contributed to retain a known blocked-via failure; no routing result is a condition for retaining it.
...
`npm test` verifies benchmark bytes, preserved originals, exact declared correction, extracted SRJ hashes, layout uniqueness, exact 25+25+1 membership/order, provenance, sizes, and top-side eligibility. `validate:upstream` additionally downloads all 51 immutable source files and confirms equality with the unmodified samples or preserved original. GitHub Actions checks local integrity on each push and pull request.
...
Curation selects from the exact pinned Git trees and file bytes, independent of each checkout's current branch or uncommitted changes, then reproducibly applies the declared correction and preserves its original. Contributed inputs are read from their pinned source commits in this repository; use a full Git clone or fetch `b07fa8a96acc6cf097b14de128f76ee5acf866d0` first. `--contributions-root` optionally selects another clone containing those commits. The optional output directory lets you compare a fresh reproduction without rewriting the checked-in dataset. Omit `--output-root` to update this repository intentionally. The older `--legacy-root` option remains compatible with existing vendored dataset01/dataset-srj18 inputs.

## Pedometer regression

`pedometer` is the original unrouted input exported from seveibar's Bluetooth
pedometer using `tsci build --site` (tscircuit 0.0.2463). It has a 42 × 26 mm
four-layer board, 66 connections, 230 terminal records, 190 obstacles, and no
preloaded traces or vias. Its SHA-256 is
`f546b05ca6dd9f60702775e0b73fdf2f49cb95002e9ac52a50c73dd6d4c892f4`.

The antenna-region keepout blocks through-vias at some top pads. This case
reproduced Happy Autorouter 0.0.5 stopping during via-in-pad preparation and
motivated a final direct fallback with warning events. The original keepout,
all electrical connections, placement, widths, and clearances are preserved.
No solver output or expected pass status is stored with the input. The source
export is preserved in `originals/pedometer.json` at the pinned contribution
commit, with an identical benchmark copy in `samples/pedometer.json`.

## Licensing

The loader, scripts, and documentation are MIT licensed. The pedometer input is contributed by seveibar under MIT with its notice in `licenses/pedometer-MIT.txt`. Original sample rights remain with upstream authors. MIT notices for dataset01 and upstream autorouter report fixtures are retained in `licenses/`. Dataset-srj18 provides no repository-wide license at its pinned commit; the two selected samples derive from Antmicro boards licensed Apache-2.0. Original Apache notices and `source-files.json` attribution are retained. Those upstream board URLs use `main`; the actual exported SRJ bytes are pinned to the dataset commit. See [licenses/README.md](licenses/README.md).
...
| `pedometer` | 4 | 66 | [originals/pedometer.json](https://github.com/tscircuit/dataset-srj34-single-side-four-layer/blob/b07fa8a96acc6cf097b14de128f76ee5acf866d0/originals/pedometer.json) |
